...
module.exports = class StrategyManager {
  ...
  async getSentimentBinanceFuturres(symbol) {
    const [topTraders, globalTraders] = await Promise.all([
      fetch(
        'https://fapi.binance.com/futures/data/topLongShortPositionRatio?symbol=' + symbol + '&period=10m&limit=2'
      ),
      fetch(
        'https://fapi.binance.com/futures/data/globalLongShortAccountRatio?symbol=' + symbol + '&period=10m&limit=2'
      ),
    ]);
